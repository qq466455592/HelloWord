...
public class ImageFetcher extends ImageResizer
{
    ...
    private void checkConnection(Context context)
    {
        ...
        if (networkInfo == null || !networkInfo.isConnectedOrConnecting())
        {
            ...
            MyLog.e(TAG, "checkConnection - no connection found");
        }
    }
    ...
    {
        MyLog.d(TAG, "processBitmap - " + data);
    ...
    {
        File cacheDir = DiskLruCache.gethttpCacheDir(context, HTTP_CACHE_DIR);

        DiskLruCache cache = DiskLruCache.openCache(context, cacheDir);
        if (cache == null)
        {
            cacheDir = DiskLruCache.getInternalDir(context, HTTP_CACHE_DIR);
            cache = DiskLruCache.openCache(context, cacheDir);
        }

        final File cacheFile = new File(cache.createFilePath(urlString));

        MyLog.d(TAG, "downloadBitmap - downloading - " + urlString);
        ...
        try
        {
            ...
            String netType = FunctionUtil.getNetType(context);
...
//            {
                ...
                if (new File(cacheFile + ".temp").exists())
                {
                    new File(cacheFile + ".temp").renameTo(cacheFile);
                }
//            }
//            else
//            {
//                LogSuning.e("-------------����ͼƬ����300KB-----------------");
//            }
